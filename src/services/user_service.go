...
func (db *DB) UpdateUser(id int, updateUser models.User) (err error) {
	hashedPassword, err := bcrypt.GenerateFromPassword([]byte(updateUser.Password), 8)
	_, err = db.Exec("UPDATE user_table SET email = $1, login = $2,"+
		"fullname = $3, password = $4, acc_verified = $5"+
		"where id = $6", updateUser.Email,
		updateUser.Login, updateUser.Fullname, hashedPassword,
		updateUser.AccVerified, id)
	if err != nil {
		return err
	}
	return nil
